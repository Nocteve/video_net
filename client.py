...
#         """实时显示性能指标"""
#         current_time = time.time()
#         if current_time - PERF_MONITOR["last_frame_time"] >= 5:
...
import numpy as np
import struct
from collections import deque  # 新增时间戳队列
from reedsolo import RSCodec
...
class VideoClient:
    def __init__(self, server_ip='127.0.0.1', port=9999):
        self.server_ip = server_ip
        self.port = port
        self.rs = RSCodec(1)  # 基础冗余20字节
        self.last_frame = np.zeros((720, 1280, 3), dtype=np.uint8)  # 防黑屏缓存
        self.buffer = {}  # {frame_id: {chunk_index: data}}
        self.expected_frames = {}  # {frame_id: total_chunks}
        self.reconnect_count = 0
        self.frame_timestamps = deque(maxlen=30)  # 存储最近30帧的时间戳（帧率计算核心）
        self.last_calculated_fps = 0.0  # 动态计算的帧率值
        self.count=0
        self._init_socket()
        threading.Thread(target=self._send_heartbeat, daemon=True).start()
        ...
        while True:
            try:
                ...
                if len(self.buffer[frame_id]) == total_chunks:
                    ...
                    # ====== 【帧率计算优化核心】 ====== #
                    current_time = time.time()
                    self.frame_timestamps.append(current_time)  # 记录当前帧到达时间
                    
                    # 基于时间窗口计算实时帧率（至少需2个时间戳）
                    if len(self.frame_timestamps) >= 20:
                        time_span = self.frame_timestamps[-1] - self.frame_timestamps[0]
                        actual_fps = (len(self.frame_timestamps) - 1) / time_span
                        self.last_calculated_fps = actual_fps
                        self.count+=1
                    if self.count>=20:
                        print(actual_fps)
                        self.count=0
    ...
    def _show_perf_panel(self):
        """每秒更新帧率显示（非累计计数）"""
        current_time = time.time()
        if current_time - self.last_perf_update >= 1.0:  # 每秒刷新一次
            display_frame = self.last_frame.copy()
            # 显示动态计算的帧率值
            cv2.putText(display_frame, f"FPS: {self.last_calculated_fps:.1f}", (10, 30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
            cv2.putText(display_frame, f"Reconn: {self.reconnect_count}", (10, 70), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
            cv2.imshow("Video Stream", display_frame)
            self.last_perf_update = current_time  # 记录本次刷新时间
...
if __name__ == "__main__":
    client = VideoClient(server_ip="127.0.0.1")
    client.last_perf_update = time.time()  # 初始化性能面板更新时间
    client.start()